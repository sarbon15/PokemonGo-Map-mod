...
transform_from_wgs_to_gcj(Location(Fort.Latitude, Fort.Longitude))
    ...
    for poke in visible:
        ...
        bearing = get_bearing(origin_lon, origin_lat, poke.Longitude, poke.Latitude)
        
        new_poke = {
            "lat": poke.Latitude,
            "lng": poke.Longitude,
            "dist": int(gpxpy.geo.haversine_distance(origin_lat, origin_lon, poke.Latitude, poke.Longitude) * 3.28084),
            #"dist": get_distance(origin_lon, origin_lat, poke.Longitude, poke.Latitude),
            #"dist": get_simple_distance(origin_lon, origin_lat, poke.Longitude, poke.Latitude),
            "bear": bearing,
            "dir": get_direction(bearing),
            "disappear_time": disappear_timestamp,
            "id": poke.pokemon.PokemonId,
            "name": pokename
        }
        
        poke_key = '#{: >3d} {: <13s}'.format(poke.pokemon.PokemonId, replace_gender_symbols(pokename.encode('utf-8')))
        
        if poke_key not in pokes_by_id:
    	    pokes_by_id[poke_key] = dict()
        pokes_by_id[poke_key][poke.SpawnPointId] = new_poke
        
        pokemons[poke.SpawnPointId] = new_poke
        
    clear_stale_pokemons()
        
    print('\n[+] Current pokemon near your location:')
    for mon_key in sorted(pokes_by_id.keys()):
...
def replace_gender_symbols(string):
    return string.replace("♀", "[F]").replace("♂", "[M]")
...
def get_minutes_left(disappear_time):
    then = datetime.fromtimestamp(disappear_time)
    now = datetime.now()
    mins_secs = divmod((then - now).total_seconds(), 60)
    mins = mins_secs[0] + round(mins_secs[1] / 60, 1)
    return mins
    
def get_direction(bearing):
    interval = 22.5
    half_int = interval / 2
    dir_list = ['N', 'NNW', 'NW', 'WNW', \
                'W', 'WSW', 'SW', 'SSW', \
                'S', 'SSE', 'SE', 'ESE', \
                'E', 'ENE', 'NE', 'NNE']
    
    for it in range(0, 16):
        if bearing <= ((it * interval) + half_int):
            return dir_list[it]
    return dir_list[0]
...
#                pokemon['name'].encode('utf-8'), pokemon['lat'], FLOAT_LONG - pokemon['lng'])
            del pokemons[pokemon_key]
                
    for mon_key in pokes_by_id.keys():
        current_mon = pokes_by_id[mon_key]
        for info_keys in current_mon.keys():
            poke_info = current_mon[info_keys]
            if current_time > poke_info['disappear_time']:
                print "[+] removing stale pokemon {:s} at [{:d}:{:s}] from list".format(replace_gender_symbols(poke_info['name'].encode('utf-8')), poke_info['dist'], poke_info['dir'])
                del current_mon[info_keys]
        if len(current_mon) == 0:
            del pokes_by_id[mon_key]
